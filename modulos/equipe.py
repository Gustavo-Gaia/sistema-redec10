...
def painel_equipe(aba):
    with aba:
        ...
        def card(titulo, nomes):
            # Fonte levemente menor se houver muitos nomes para evitar quebra feia
            estilo_fonte = "12px" if len(nomes) <= 1 else "11px"
            
            conteudo = ""
            if nomes:
                for n in nomes:
                    # Adicionada borda sutil e margem para separar nomes grandes
            ...
            <div style="
                ...
                <div style="font-size: 10px; text-transform: uppercase; opacity: 0.8; font-weight: 700; margin-bottom: 12px; letter-spacing: 0.5px; border-bottom: 1px solid rgba(255,255,255,0.3); padding-bottom: 5px;">
                    {titulo}
                </div>
                <div style="font-size: {estilo_fonte}; font-weight: 500; flex-grow: 1; display: flex; flex-direction: column; justify-content: center;">
                    {conteudo}
                </div>
            </div>
...
def relatorios(aba):
    with aba:
        ...
        # Usamos o nunique para contar membros únicos ocupando funções no momento
        ...
        col3.metric("Ativos no Sistema", len(df_equipe[df_equipe["ativo"] == True]))
        
        st.divider()
        st.markdown("### 🖨 Exportar Dados")
        
        # Correção do NameError: Utilizando df_equipe para exportação
        st.download_button(
